...
flowchart LR
    ...
    subgraph CMS["apps/cms — Strapi (Render)"]
...
| `CLOUDINARY_NAME` / `CLOUDINARY_KEY` / `CLOUDINARY_SECRET`                                              | Set all three to switch media storage from local disk to Cloudinary — required in production since most free-tier hosts (Render included) don't reliably persist local disk across redeploys |
| `PUBLIC_URL`                                                                                            | Public HTTPS URL of the deployed CMS. Without it, Strapi builds media/admin URLs from its internal bind address instead of the real domain                                                   |
...
**`apps/cms` → Render** (free tier: Web Service + PostgreSQL, no card required)

1. Provision a Render PostgreSQL instance; set `DATABASE_CLIENT=postgres`, `DATABASE_URL` (its
   internal connection string), and `DATABASE_SSL=true`.
2. Build/start commands run from the **repo root**, not `apps/cms` — this is an npm workspace, so
   installing from the subfolder alone won't resolve correctly:
   `npm install && npm run build --workspace=apps/cms` / `npm run start --workspace=apps/cms`.
3. Set the three `CLOUDINARY_*` vars — required since Render's free-tier disk doesn't reliably
   survive redeploys, so local-disk uploads would be lost.
4. Set `PUBLIC_URL` to the deployed Render URL — without it, Strapi builds media/admin URLs from
   its internal bind address instead of the public domain.
5. Set `CORS_ORIGIN` to the deployed Vercel URL, and add the revalidation webhook (Strapi admin →
   Settings → Webhooks) pointed at `<vercel-url>/api/revalidate?secret=<REVALIDATE_SECRET>`.
6. To move local content over, Strapi's built-in transfer tool works well for content + small
   media in one pass: `npx strapi transfer --to <render-url>/admin --to-token <transfer-token>`
   (run from `apps/cms` locally). Very large individual files (e.g. a background video) can hit a
   timeout on Render's free tier mid-transfer — upload those directly through the Render admin's
...
| Deployment (Vercel + CMS host)   | ✅ live — see [Deployment](#deployment)               |
